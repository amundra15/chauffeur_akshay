...
class configMain:
	...
	def __init__(self):
		...
		self.variable_names = ['Steer','Gas','Brake','Hand_B','Reverse','Steer_N','Gas_N','Brake_N','Pos_X','Pos_Y','Speed',\
						'C_Gen','C_Ped','C_Car','Road_I','Side_I','Acc_x','Acc_y','Acc_z','Plat_Ts','Game_Ts','Ori_X','Ori_Y','Ori_Z','Control']
		...
		self.models_path = 'models/carla_3'
...
class configOutput(configMain):
	...
	def __init__(self):
		...
		self.check_images =False  # If we want to have an aux screen watching the training images
	

		self.targets_to_print = ['Steer','Gas'] # Also prints the error. Maybe Energy
		self.steer_branch = 0  # for the steer case we also have to select the branch
		self.inputs_to_print = []
