...
class configMain:
	...
	def __init__(self):
		...
		self.dropout = [0.8,0.8,0.8,0.8,0.4,0.4,0.4,0.4,0.4,0.4,0.4,0.4,0.4]
		#self.dropout = [1,1,1,1,1,1,1,1]
		
		self.restore = True # This is if you want to restore a saved model

		self.models_path = os.path.join('models', os.path.basename(__file__)[:-3])
...
class configInput(configMain):
	...
	def __init__(self):
		...
		oc = lambda aug: iaa.Sometimes(0.7, aug)
		rl = lambda aug: iaa.Sometimes(0.7, aug)
		self.augment = iaa.Sequential([


	        rl(iaa.GaussianBlur((0, 3))), # blur images with a sigma between 0 and 1.5
	        rl(iaa.AdditiveGaussianNoise(loc=0, scale=(0.0, 0.2), per_channel=0.5)), # add gaussian noise to images
	        rl(iaa.Dropout((0.0, 0.2), per_channel=0.5)), # randomly remove up to X% of the pixels
	        oc(iaa.Add((-40, 40), per_channel=0.5)), # change brightness of images (by -X to Y of original value)
	        st(iaa.Multiply((0.2, 3), per_channel=0.2)), # change brightness of images (X-Y% of original value)
	        rl(iaa.ContrastNormalization((0.3, 2), per_channel=0.5)), # improve or worsen the contrast
