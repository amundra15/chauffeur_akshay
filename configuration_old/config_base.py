...
class configMain:
	...
	def __init__(self):
		...
		self.variable_names = ['Steer','Gas','Steer_N','Acc_N','Speed','Yaw_S','Yaw','Pos_X','Pos_Y','Time','Control','Noise']
		# _N is noise, Yaw_S is angular speed

		self.targets_names =['Steer','Gas','Speed']
		self.targets_sizes = [1,1,1]
		...
		self.models_path = 'models/control_input_pred_cat/'
		self.train_path_write = 'models/control_input_pred_cat/train/'
		self.val_path_write = 'models/control_input_pred_cat/val/'
...
class configInput(configMain):
	...
	def __init__(self):
		...
		self.valid_pos_train = range(0,1730) 

		self.valid_pos_val = range(0,199) # validation data is in a different folder
		...
		self.dataset_names = ['targets','control']
...
class configTrain(configMain):
	...
	def __init__(self):
		...
		self.lambda_l2 = 1e-3
		self.output_weigth = [0.45,0.45,0.45,0.45,0.35,0.2]
		self.network_name = 'controlNetSpeedP'	
		self.lambda_tolerance = 1
...
class configMain:
	...
	def __init__(self):
		...
		self.sequence_size_lstm = 0
		self.fusion_size =1
		self.sequence_size =(self.fusion_size)*2
		self.sequence_resample  =  self.fusion_size 


		self.resample_stride = self.sequence_size/self.sequence_resample
		self.sequence_stride =  self.sequence_size/2
		self.number_of_sequences = self.batch_size/(self.sequence_size_lstm+1)
		#self.input_size = (227,227,3)
		#self.manager_name = 'control_speed'
		# y , x
		self.image_size = (100,200,3)
		self.network_input_size = (100,200,3)
		self.variable_names = ['Steer','Acc','Steer_N','Acc_N','Speed','Yaw_S','Yaw','Pos_X','Pos_Y','Time','Control','Noise']


		self.targets_names =['Steer','Acc','Speed']
		self.targets_sizes = [1,1,1]
		...
		self.branch_config = [["Steer"],["Steer"],["Steer"],["Steer"],["Acc"],["Speed"]]
