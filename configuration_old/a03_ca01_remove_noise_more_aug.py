...
class configInput(configMain):
	...
	def __init__(self):
		...
		self.augment = iaa.Sequential([
	        ...
	        rl(iaa.GaussianBlur((0, 2.5))), # blur images with a sigma between 0 and 1.5
	        rl(iaa.AdditiveGaussianNoise(loc=0, scale=(0.0, 0.25), per_channel=0.5)), # add gaussian noise to images
	        rl(iaa.Dropout((0.0, 0.2), per_channel=0.5)), # randomly remove up to X% of the pixels
	        oc(iaa.Add((-50, 50), per_channel=0.5)), # change brightness of images (by -X to Y of original value)
	        st(iaa.Multiply((0.25, 2.5), per_channel=0.2)), # change brightness of images (X-Y% of original value)
	        rl(iaa.ContrastNormalization((0.4, 2), per_channel=0.5)), # improve or worsen the contrast
