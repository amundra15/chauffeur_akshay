...
'''def mse_branched_boostgas(network_outputs,ground_truths,control_input,config):
	...
	for i in range(len(branches_configuration)):
		for j in range(len(branches_configuration[i])):
			...
			print network_outputs_split
			target_name = branches_configuration[i][j] # name of the target
			target_gt = ground_truths[config.targets_names.index(target_name)]
			squared_dist = tf.pow(tf.subtract(target_gt, network_outputs_split),2)
			dist =tf.abs(tf.subtract(target_gt,  network_outputs_split))
			variables_loss_vec.append(dist)

			if i < config.number_steering_branches:
				branch_selection = tf.reshape(control_input[:,i],tf.shape(ground_truths[0]))
			else:
				branch_selection =tf.ones(tf.shape(branch_selection))

			if target_name == "Gas":
				variable_loss =(squared_dist +  squared_dist*tf.abs(target_gt)*config.lambda_tolerance )* branch_selection 
			else:
				variable_loss =squared_dist * branch_selection 

			variable_loss =squared_dist * branch_selection 

			difference_vec.append(variable_loss)
			if i==0 and j==0:
				loss_function = variable_loss*config.output_weigth[i]
			else:
				loss_function = tf.add(variable_loss*config.output_weigth[i],loss_function)

	return loss_function,variables_loss_vec,difference_vec,None,branch_selection

def mse_branched_boostpedals(network_outputs,ground_truths,control_input,config):
	...
	for i in range(len(branches_configuration)):
		...
		for j in range(len(branches_configuration[i])):
			...
			if target_name == "Gas":
				...
				variable_loss =(squared_dist +  squared_dist*tf.abs(target_gt)*config.lambda_tolerance*2 )* branch_selection 
			else:
				variable_loss =squared_dist * branch_selection
...
def mse_branched_boost2gas(network_outputs,ground_truths,control_input,config):
	...
	for i in range(len(branches_configuration)):
		...
		for j in range(len(branches_configuration[i])):
			...
			if target_name == "Gas":
				variable_loss =(squared_dist +  squared_dist*tf.pow(target_gt,2)*config.lambda_tolerance )* branch_selection 
			else:
				variable_loss =squared_dist * branch_selection 


			energy_vec.append(variable_loss)
			if i==0 and j==0:
				loss_function = variable_loss*config.output_weigth[i]
			else:
				loss_function = tf.add(variable_loss*config.output_weigth[i],loss_function)
...
def mse_branched_ladd(network_outputs,ground_truths,control_input,config):
