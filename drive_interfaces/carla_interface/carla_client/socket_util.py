import binascii
import socket
import struct
import logging

def int2bytes(i):
    hex_string = '%x' % i
    n = len(hex_string)
    return binascii.unhexlify(hex_string.zfill(n + (n & 1)))

def bytes2int(str):
	return int(str.encode('hex'), 16)
	
def connect(host,port):

	try:
		sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	except socket.error, msg:
		print ("Error Creating Socket World %s", msg)
		sys.exit()

	sock.connect((host,port))

	return sock


def send_message(sock, message):
	""" Send a serialized message (protobuf Message interface)
	    to a socket, prepended by its length packed in 4
	    bytes (big endian).
	"""

	s = message.SerializeToString()
	packed_len =struct.pack('>L', len(s)) #int2bytes(len(s))
	logging.debug( "SOCKET Send: %d bytes" % int(len(s)))
	#print repr(packed_len)
	sock.sendall(packed_len + s)
	#sock.sendall(packed_len)
	#sock.shutdown(socket.SHUT_RDWR)
	#sock.close()



def get_message(sock):
	""" Read a message from a socket.
	"""


	len_buf = socket_read_n(sock, 4)
	logging.debug( "SOCKET RECEIVED: %d bytes" % bytes2int(len_buf))
	msg_len = struct.unpack('>L', len_buf)[0]
	#print msg_len

	msg_buf = socket_read_n(sock, msg_len)
	#sock.shutdown(socket.SHUT_RDWR)
	#sock.close()

	return msg_buf


def socket_read_n(sock,n):
    """ Read exactly n bytes from the socket.
        Raise RuntimeError if the connection closed before
        n bytes were read.
    """

    buf = ''
    while n > 0:
    	#print "RECEIVE"
    	
        data = sock.recv(n)
        if data == '':
            raise RuntimeError('unexpected connection close')
        buf += data
        n -= len(data)

    #print buf
    #logging.debug( "SOCKET RECEIVED: %s" % buf)

    return buf



